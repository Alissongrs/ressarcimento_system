...
const asDateInput = (val) => {
  ...
  try {
    ...
    return iso.startsWith('0001-01-01') ? '' : iso;
  } catch {
    return '';
  }
};
...
const normalizeCreditoValue = (val) => {
  ...
  // Normalizar formato decimal para garantir 2 casas
...
const buildFaturamentoPayload = (fat) => {
  const arr = Array.isArray(fat) ? fat : [];
  return arr
    .map((it) => ({
...
const ProcessoDetalhes = () => {
  ...
  const getItemDate = (h) => {
    ...
    // Try to parse DD/MM/AAAA format
    const brMatch = String(raw).match(/^(\d{2})\/(\d{2})\/(\d{4})/);
    if (brMatch) {
      const [, d, m, y] = brMatch;
      const d_obj = new Date(`${y}-${m}-${d}`);
      return isNaN(d_obj.getTime()) ? 0 : d_obj.getTime();
    }
  ...
  useProcessoSSE(id, sseToken, (payload) => {
    ...
    if (Object.prototype.hasOwnProperty.call(payload, 'historico_updated')) {
      carregarHistorico();
    } else {
      carregarHistorico();
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const deferimento = {
        ...
        credito_simples: toNumberBR(normalizeCreditoValue(creditoSimples)),
        credito_dobro: habilitarCreditoDobro ? toNumberBR(normalizeCreditoValue(creditoDobro)) : 0,
        data_credito_dobro:
      ...
      // Fluxo de Ressarcimento
      const fluxoPayload = buildFluxoPayload(fluxoRessarcimento);
      if (fluxoPayload.length > 0) {
        fd.append('fluxo_ressarcimento', JSON.stringify({ itens: fluxoPayload }));
      }

      // Faturamento
      const faturamentoPayload = buildFaturamentoPayload(faturamento);
