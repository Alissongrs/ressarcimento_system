...
export default function ChatRequisicao() {
  ...
  const [messages, setMessages] = useState([
    ...
    { role: BOT, text: 'Para comecarmos, voce poderia me enviar o numero da sua UC (apenas digitos)? Se nao tiver agora, tudo bem — podemos seguir assim mesmo.' },
  ]);
  ...
  const parseMeses = (s) => {
    ...
    for (const part of parts) {
      ...
      // suporta intervalos: "mm-yyyy a mm-yyyy" | "mm/yyyy at� mm/yyyy" | "mm/yyyy ate mm/yyyy"
      let token = pLow.replace('at�', 'a').replace('ate', 'a');
      if (token.includes(' a ')) {
        const [iniRaw, fimRaw] = part.replace(/at�|ate/gi, 'a').split(/\sa\s/);
  ...
  const handleSend = async () => {
    ...
    try {
      if (step === 0) {
        ...
        push(BOT, 'Perfeito! Para quais meses devemos olhar? Voc� pode enviar algo como "07/2024, 08/2024" ou um intervalo "07/2024 a 09/2024".');
        setStep(1);
        return;
      }
      ...
      if (step === 1) {
        ...
        if (meses.length === 0) { push(BOT, 'Envie meses v�lidos (separe por v�rgula) ou um intervalo. Ex.: 07/2024, 08/2024 ou 07/2024 a 09/2024'); setBusy(false); return; }
        setPeriodos(meses);
        try {
          // 1) Tenta via alias p�blico /faturas-uc (usa unidade + meses)
          const dados = await buscarFaturasPorUnidadeMeses(form.uc, mesesRefs(meses));
          // Normaliza poss�veis formatos de resposta do backend
          let det = [];
          if (Array.isArray(dados?.faturas)) {
            ...
            // Caso singular (quando apenas um m�s foi pedido)
