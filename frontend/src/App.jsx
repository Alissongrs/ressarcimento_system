...
function App() {
  ...
  useEffect(() => {
    ...
    es.addEventListener('alerta_boot', (ev) => {
      try {
    ...
    es.addEventListener('alerta_novo', (ev) => {
      ...
      try {
        ...
        const msg = uc || cliente
          ? `Nova Requisicao pendente${uc ? ' — UC ' + uc : ''}${cliente ? ' — ' + cliente : ''}`
          : 'Nova Requisicao pendente';

        setToast({ open: true, type: 'info', text: msg });
      } catch {
        setToast({ open: true, type: 'info', text: 'Nova Requisicao pendente' });
      }
    });
    ...
    es.addEventListener('requisicao_nova', (ev) => {
      try {
        ...
        setToast({ open: true, type: 'success', text: `Requisicao criada${id ? ' — #' + id : ''}` });
      } catch {
        setToast({ open: true, type: 'success', text: 'Requisicao criada' });
      }
    });

    es.addEventListener('alerta_unread', (ev) => {
      try {
        const data = JSON.parse(ev.data || '{}');
        if (typeof data.unread === 'number') setUnread(data.unread);
      } catch {}
  ...
  useEffect(() => {
    if (!user) return;
    if (role === 'gestor' && !gestorVencShownRef.current) {
  ...
  const AdminSidebar = () => (
    <aside
      className="sticky top-0 sidebar-collapsible p-4 border-r h-[100vh] overflow-hidden shrink-0 flex flex-col rounded-tr-3xl rounded-br-3xl"
    ...
    >
      <div className="relative flex flex-col h-full">
        ...
        <nav className="space-y-1 flex-1 overflow-visible">
  <NavLink to="/" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Inicio">
    <Home className="w-5 h-5" strokeWidth={1.25} />
    <span className="text-sm font-medium sidebar-label">Inicio</span>
  </NavLink>
  <NavLink to="/novo" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Nova Requisicao">
    <FilePlus className="w-5 h-5" strokeWidth={1.6} />
    <span className="text-sm font-medium sidebar-label">Nova Requisicao</span>
  </NavLink>
  <NavLink to="/Requisicoes" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Requisições">
    <ClipboardList className="w-5 h-5" strokeWidth={1.25} />
    <span className="text-sm font-medium sidebar-label">Requisições</span>
  </NavLink>
  <NavLink to="/processos" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Processos">
    <ClipboardList className="w-5 h-5" strokeWidth={1.25} />
    <span className="text-sm font-medium sidebar-label">Processos</span>
  </NavLink>
  <NavLink to="/historico" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Histórico">
    <History className="w-5 h-5" strokeWidth={1.25} />
    <span className="text-sm font-medium sidebar-label">Histórico</span>
  </NavLink>
  <NavLink to="/dashboard" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Relatórios">
    <BarChart3 className="w-5 h-5" strokeWidth={1.25} />
    <span className="text-sm font-medium sidebar-label">Relatórios</span>
  </NavLink>
  <NavLink to="/auditoria" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Auditoria">
    <Receipt className="w-5 h-5" strokeWidth={1.25} />
    <span className="text-sm font-medium sidebar-label">Auditoria</span>
  </NavLink>
  <NavLink to="/regras" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Regras">
    <FilePenLine className="w-5 h-5" strokeWidth={1.25} />
    <span className="text-sm font-medium sidebar-label">Regras</span>
  </NavLink>
  <NavLink to="/ocr" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="OCR">
    <ClipboardList className="w-5 h-5" strokeWidth={1.25} />
    <span className="text-sm font-medium sidebar-label">OCR</span>
  </NavLink>
  <NavLink to="/caixa-de-email" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Caixa de Email">
    <MailIcon className="w-5 h-5" strokeWidth={1.25} />
    <span className="text-sm font-medium sidebar-label">Caixa de Email</span>
  </NavLink>
  <NavLink to="/chat-ia" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Chat IA">
    <Bot className="w-5 h-5" strokeWidth={1.25} />
    <span className="text-sm font-medium sidebar-label">Chat IA</span>
  </NavLink>
  {role === 'admin' && (
    <>
      <NavLink to="/adminfeedbacks" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Feedbacks (Admin)">
        <Megaphone className="w-5 h-5" strokeWidth={1.6} />
        <span className="text-sm font-medium sidebar-label">Feedbacks</span>
      </NavLink>
      <NavLink to="/admin/prazos" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Prazos & Alarmes (Admin)">
        <AlarmClock className="w-5 h-5" strokeWidth={1.6} />
        <span className="text-sm font-medium sidebar-label">Prazos & Alarmes</span>
      </NavLink>
      <NavLink to="/admin/editor" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Editor (Admin)">
        <FilePenLine className="w-5 h-5" strokeWidth={1.6} />
  ...
  <NavLink to="/Requisicoes" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Requisições">
    <ClipboardList className="w-8 h-8 shrink-0" strokeWidth={1.25} />
    <span className="text-sm font-medium sidebar-label">Requisições</span>
  </NavLink>
  <NavLink to="/processos" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Processos">
    <ClipboardList className="w-8 h-8 shrink-0" strokeWidth={1.25} />
  ...
  <NavLink to="/regras" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="Regras">
    <FilePenLine className="w-8 h-8 shrink-0" strokeWidth={1.25} />
    <span className="text-sm font-medium sidebar-label">Regras</span>
  </NavLink>
  <NavLink to="/ocr" className={({ isActive }) => `w-full flex items-center gap-3 px-3 py-2 rounded-md ${isActive ? 'bg-[var(--menu-hover)] text-[var(--menu-fg)]' : 'hover:bg-[var(--menu-hover)]'}`} title="OCR">
    <ClipboardList className="w-8 h-8 shrink-0" strokeWidth={1.25} />
  ...
  return (
    <ThemeProvider>
      <Router>
              ...
              <Routes>
                ...
                {user && (
                  <>
                    <Route
                      path="/"
                      element={(role === 'gestor' || role === 'admin')
                        ? <HomeGestorAdmin />
                        : <ChatRequisicao />
                      }
                    />
                    ...
                    <Route path="/requisicoes/:id" element={<DetalhesRequisicao />} />
                    ...
                    {(role === 'gestor' || role === 'admin') ? (
                      <>
                        ...
                        {/* Páginas internas de auditoria/ocr/regras etc. só gestor/admin */}
                      ...
                      <>
                        {/* bloqueios para usuários comuns */}
                    ...
                    {/* Auditoria acessível a qualquer usuário logado */}
            ...
            <FeedbackModal
              ...
              onSubmit={async (msg) => {
                try {
                  ...
                  setToast({ open: true, type: 'success', text: 'Feedback enviado com sucesso!' });
                } catch {
                  setToast({ open: true, type: 'error', text: 'Falha ao enviar feedback.' });
                }
              }}
            />
          </main>
        ...
        <AlertasVencimentoModal open={openGestorVenc} onClose={() => setOpenGestorVenc(false)} />
...
function RouteKeeper({ user }) {
  ...
  // Save last non-root route while logged in
  useEffect(() => {
    if (!user) return;
  ...
  // Restore once when landing on root
  useEffect(() => {
    if (!user || restoreRef.current) return;
    const isRoot = location.pathname === '/';
    try {
      const last = localStorage.getItem('lastRoute');
      if (last && isRoot && last !== '/' && last !== '/login' && last !== location.pathname) {
