...
# Configura caminho do Tesseract (Linux: PATH por padrão; env opcional)
...
# Caminho opcional do poppler (Linux: vazio = usar utilitários do sistema no PATH)
POPPLER_PATH = (os.getenv("POPPLER_PATH", "") or "").strip()

# TESSDATA_PREFIX opcional para idiomas do Tesseract
TESSDATA_PREFIX = (os.getenv("TESSDATA_PREFIX", "") or "").strip()
if TESSDATA_PREFIX:
    os.environ["TESSDATA_PREFIX"] = TESSDATA_PREFIX

print(f"[ocr_service] TESSERACT_CMD = {TESSERACT_CMD or 'PATH'}")
print(f"[ocr_service] TESSDATA_PREFIX = {TESSDATA_PREFIX or '(não definido)'}")
print(f"[ocr_service] POPPLER_PATH = {POPPLER_PATH or 'PATH'}")
...
if PaddleOCR is not None:
    try:
        PADDLE_READER = PaddleOCR(use_angle_cls=True, lang="pt")
