...
func CreateTag(c *gin.Context) {
	...
	var existenteID int
	err := database.DB_App.QueryRow("SELECT id_tag FROM DM_TAGS WHERE nome = ?", newTag.Nome).Scan(&existenteID)
	...
	result, err := database.DB_App.Exec(query, newTag.Nome, newTag.Cor)
	if err != nil {
...
func UpdateProcessoTags(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&tagIDs); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Corpo da requisição inválido, esperando um array de IDs"})
		return
	}
	...
	if _, err := tx.Exec("DELETE FROM FT_PROCESSO_TAGS WHERE id_processo = ?", processoID); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Erro ao limpar tags antigas"})
		return
	}

	if len(tagIDs) > 0 {
		stmt, err := tx.Prepare("INSERT INTO FT_PROCESSO_TAGS (id_processo, id_tag) VALUES (?, ?)")
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Erro ao preparar inserção de tags"})
			return
		}
		defer stmt.Close()

		for _, tagID := range tagIDs {
			if _, err := stmt.Exec(processoID, tagID); err != nil {
