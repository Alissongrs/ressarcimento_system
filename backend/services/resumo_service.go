...
func (s *ResumoService) callOpenAI(ctx context.Context, system, user string) (string, error) {
    ...
    if apiKey == "" { return "",  Err("OPENAI_API_KEY não definido") }
    body := map[string]any{
        "model": "gpt-4o-mini",
    ...
    var out struct{ Choices []struct{ Message struct{ Content string `json:"content"` } `json:"message"` } `json:"choices"`; Error map[string]any `json:"error"` }
    _ = json.NewDecoder(resp.Body).Decode(&out)
    if resp.StatusCode >= 400 { return "", Errf("openai %d: %v", resp.StatusCode, out.Error) }
    if len(out.Choices) == 0 { return "", Err("sem escolhas da openai") }
...
func (s *ResumoService) buildBatchPrompt(ctx context.Context, ids []int64) (system string, user string, items map[int64]any) {
    ...
    for _, pid := range ids {
        ...
        arr := make([]hist,0,20)
        for rows!=nil && rows.Next(){
            var it hist; _ = rows.Scan(&it.Data,&it.Etapa,&it.Sub,&it.Coment,&it.Tipo); arr = append(arr,it)
        }
        if rows!=nil { rows.Close() }
        items[pid] = map[string]any{"id": pid, "historico": arr}
    }
    // user message
    msg := struct{Items []any `json:"items"`}{ Items: make([]any,0,len(items)) }
    for _, pid := range ids { msg.Items = append(msg.Items, items[pid]) }
...
func (s *ResumoService) callOpenAIJSON(ctx context.Context, system, user string) (map[string]any, error) {
    apiKey := os.Getenv("OPENAI_API_KEY")
    if apiKey == "" { return nil,  Err("OPENAI_API_KEY não definido") }
    body := map[string]any{
        "model": "gpt-4o-mini",
        "messages": []map[string]string{
            {"role":"system","content":system},
    ...
    var out struct{ Choices []struct{ Message struct{ Content string `json:"content"` } `json:"message"` } `json:"choices"`; Error map[string]any `json:"error"` }
    _ = json.NewDecoder(resp.Body).Decode(&out)
    if resp.StatusCode >= 400 { return nil, Errf("openai %d: %v", resp.StatusCode, out.Error) }
    if len(out.Choices) == 0 { return nil, Err("sem escolhas da openai") }
...
func (s *ResumoService) ProcessarResumos(ctx context.Context, limit int) error {
    if err := s.Repo.EnsureTable(ctx); err != nil { return err }
    ...
    for i := 0; i < len(ids); i += batchSize {
        end := i + batchSize; if end > len(ids) { end = len(ids) }
        batch := ids[i:end]
        ...
        if err != nil {
            ...
            for _, id := range batch { _ = s.Repo.SaveError(ctx, id, err.Error()) }
            // log batch error
        ...
        if !ok {
            for _, id := range batch { _ = s.Repo.SaveError(ctx, id, "resposta sem summaries") }
        ...
        for _, raw := range summaries {
            ...
            var pid int64
            switch v := m["id"].(type) {
            case float64: pid = int64(v)
            case int64: pid = v
            }
            if pid == 0 { continue }
            sum, _ := m["summary"].(string)
            if sum == "" { _ = s.Repo.SaveError(ctx, pid, "summary vazio"); seen[pid]=true; errIDs = append(errIDs,pid); continue }
        ...
        for _, id := range batch {
            if !seen[id] { _ = s.Repo.SaveError(ctx, id, "sem retorno no batch"); errIDs = append(errIDs,id) }
        }
...
func (s *ResumoRuntimeStats) updateBatchResult(ok, err []int64) {
    s.mu.Lock()
    if len(ok) > 0 { s.LastOKIDs = append([]int64(nil), ok...) }
    if len(err) > 0 { s.LastErrIDs = append([]int64(nil), err...) }
...
func GetResumoRuntimeStatus() ResumoRuntimeStats {
    resumoStats.mu.RLock()
    defer resumoStats.mu.RUnlock()
    // deep-copy slices
    cp := ResumoRuntimeStats{
