...
func EnviarAlertaPessoalComID(alertaID int64, userID int64, mensagem, dataAlerta string) {
	...
	if t, err := time.Parse("2006-01-02", dataAlerta); err == nil {
		...
		<table cellpadding="0" cellspacing="0" style="margin-top:16px;">
		  <tr>
		    <td style="padding:10px 16px;background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0;">
		      <span style="font-size:12px;color:#64748b;">📅 Vencimento: </span>
		      <strong style="font-size:13px;color:#1e293b;">%s</strong>
		      &nbsp;&nbsp;
		      <span style="display:inline-block;background:%s22;color:%s;border:1px solid %s44;border-radius:12px;padding:2px 10px;font-size:11px;font-weight:700;">%s</span>
		    </td>
		  </tr>
		</table>`, dataAlertaFormatada, urgenciaCor, urgenciaCor, urgenciaCor, urgenciaLabel)
	}
...
<table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.10);">
  ...
  <tr>
    <td style="padding:32px 40px 24px;">
      <p style="margin:0 0 12px;font-size:12px;font-weight:700;color:#64748b;letter-spacing:1px;text-transform:uppercase;">Mensagem do lembrete</p>

      <!-- Card do lembrete -->
      <table width="100%%" cellpadding="0" cellspacing="0" style="border-radius:10px;overflow:hidden;border:1px solid #e2e8f0;">
        <tr>
          <td style="width:5px;background:#2563eb;padding:0;">&nbsp;</td>
          <td style="padding:18px 20px;background:#f8fafc;">
            <p style="margin:0;font-size:15px;color:#1e293b;line-height:1.6;font-style:italic;">&ldquo;%s&rdquo;</p>
          </td>
        </tr>
      </table>
