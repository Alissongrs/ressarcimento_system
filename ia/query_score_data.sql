...
-- FT_ALERTAS — usado no JOIN do Kanban (N+1 fix)
CALL sure_create_index('FT_ALERTAS', 'idx_alt_processo_lido', '(id_processo, lido)');
...
SELECT
    ...
    MAX(CASE WHEN hm.etapa_nova = 'Distribuidora' OR hm.etapa_anterior = 'Distribuidora' THEN 1 ELSE 0 END) AS passou_distribuidora,
    ...
    -- Label: 1 se avancou para etapas de sucesso (lookup por nome — robusto a mudanças de ID)
    CASE WHEN pr.id_coluna IN (
        SELECT id_coluna FROM DM_KANBAN_COLUNAS
        WHERE nome_coluna IN ('Fluxo de Ressarcimento', 'Faturamento', 'Concluídos')
    ) THEN 1 ELSE 0 END
        AS avancou
