...
logging.basicConfig(
    ...
    handlers = [
        ...
        logging.FileHandler("/opt/ressarcimento/ressarcimento_system/ia/pdf_pipeline.log", encoding="utf-8"),
    ],
)
...
OCR_DET = "/opt/ressarcimento/ressarcimento_system/paddleocr/det/en_PP-OCRv3_det_infer"
OCR_REC = "/opt/ressarcimento/ressarcimento_system/paddleocr/rec/latin_PP-OCRv3_rec_infer"
OCR_CLS = "/opt/ressarcimento/ressarcimento_system/paddleocr/cls/ch_ppocr_mobile_v2.0_cls_infer"
IMG_DIR  = Path("/opt/ressarcimento/ressarcimento_system/paddleocr/tmp")
...
def _parse_ocr_linhas(result) -> list[str]:
    """Extrai linhas de texto do resultado do PaddleOCR 2.x."""
    linhas = []
    if not result:
        return linhas
    for page in result:
        ...
        if isinstance(page, list):
            linhas.extend(line[1][0] for line in page if line and len(line) >= 2)
    return linhas
...
def extrair_ocr(pdf_bytes: bytes, fatura_id: int) -> tuple[str, dict]:
    ...
    try:
        ...
        for i, page in enumerate(doc):
            ...
            linhas.extend(_parse_ocr_linhas(result))
        doc.close()
    except Exception as e:
        log.warning(f"    OCR: {e}")
        return "", _campos_vazios()
...
if __name__ == "__main__":
    ...
    p.add_argument("--empresa",  type=int, nargs="+", required=True,
    ...
    main(cod_empresas=args.empresa, force=args.force, limite=args.limite,
